...
void insert_scalar_matrix_element(const int i, const int j, double const x, MATRIX_DATA* const mat);
...
void update_these_ibi_parameters(const double beta, const double chi, const dense_matrix* ref_normal_matrix, const dense_matrix* cg_normal_matrix, std::vector<double> &ref_previous_solution, std::vector<double> &previous_solution, std::vector<double> &new_solution);
...
void initialize_dense_matrix(MATRIX_DATA* const mat, ControlInputs* const control_input, CG_MODEL_DATA* const cg)
{
    ...
    mat->accumulate_virial_constraint_matrix_element = insert_dense_matrix_virial_element;
...
void initialize_accumulation_matrix(MATRIX_DATA* const mat, ControlInputs* const control_input, CG_MODEL_DATA* const cg)
{
    ...
    mat->accumulate_virial_constraint_matrix_element = insert_accumulation_matrix_virial_element;
...
void initialize_sparse_matrix(MATRIX_DATA* const mat, ControlInputs* const control_input, CG_MODEL_DATA* const cg)
{
    ...
    mat->accumulate_virial_constraint_matrix_element = insert_sparse_matrix_virial_element;
    ...
    mat->dense_fm_rhs_vector = new double[mat->fm_matrix_rows]();
    mat->ll_sparse_matrix_row_heads = new linked_list_sparse_matrix_row_head[mat->rows_less_constraint_rows];
	for(int i = 0; i < mat->rows_less_constraint_rows; i++) {
...
void determine_matrix_columns_and_rows( MATRIX_DATA* const mat, CG_MODEL_DATA* const cg, int const frames_per_traj_block, int const pressure_constraint_flag) 
{
	...
	mat->rows_less_constraint_rows = cg->n_cg_sites * frames_per_traj_block;
...
void allocate_bootstrapping(MATRIX_DATA* mat, ControlInputs* const control_input, const int rows, const int cols)
{
	...
	mat->bootstrapping_dense_fm_normal_matrices = new dense_matrix*[control_input->bootstrapping_num_estimates];
  	for (int i = 0; i < control_input->bootstrapping_num_estimates; i++) {
		mat->bootstrapping_dense_fm_normal_matrices[i] = new dense_matrix(rows, cols);
	}
	
	// rhs vectors
	mat->bootstrapping_dense_fm_normal_rhs_vectors = new double*[control_input->bootstrapping_num_estimates];
  	for (int i = 0; i < control_input->bootstrapping_num_estimates; i++) {
		mat->bootstrapping_dense_fm_normal_rhs_vectors[i] = new double[cols]();
	}
	
	// solutions
	mat->bootstrap_solutions = new std::vector<double>[mat->bootstrapping_num_estimates];
   	for (int i = 0; i < mat->bootstrapping_num_estimates; i++) {
  		mat->bootstrap_solutions[i] = std::vector<double>(cols);
	}
...
void accumulate_scalar_entropy_elements(InteractionClassComputer* const info, const int first_nonzero_basis_index, const std::vector<double> &basis_fn_vals, const int n_body, const int* particle_ids, std::array<double, DIMENSION>* const &derivatives, MATRIX_DATA * const mat)
{
   ...
   int ref_column = info->interaction_class_column_index + info->ispec->interaction_column_indices[info->index_among_matched_interactions - 1];
   int basis_columns = info->ispec->interaction_column_indices[info->index_among_matched_interactions] - info->ispec->interaction_column_indices[info->index_among_matched_interactions - 1];
   int n_cg_sites = mat->rows_less_constraint_rows / mat->frames_per_traj_block / mat->size_per_vector; // This is (n_cg_sites * frames_per_traj_block * size_per_vector) / (frames_per_traj_block * size_per_vector);
...
{
    mat->dense_fm_matrix->add_scalar(mat->rows_less_constraint_rows * mat->size_per_vector, n, x);
}
...
{
    mat->dense_fm_matrix->add_scalar(mat->rows_less_constraint_rows * mat->size_per_vector + mat->accumulation_row_shift, n, x);
}
...
void calculate_target_virial_in_dense_vector(MATRIX_DATA* const mat, double *pressure_constraint_rhs_vector)
{
    ...
    for (int k = 0; k < mat->virial_constraint_rows; k++) {
        mat->dense_fm_rhs_vector[mat->rows_less_constraint_rows * DIMENSION + k] = pressure_constraint_rhs_vector[(int)((frame_sample + k)/ mat->dynamic_state_samples_per_frame)];
        frame_sample++;
    }
}
...
void calculate_target_virial_in_accumulation_vector(MATRIX_DATA* const mat, double *pressure_constraint_rhs_vector)
{
	int frame_sample  =  mat->trajectory_block_index * mat->virial_constraint_rows;
    for (int k = 0; k < mat->virial_constraint_rows; k++) {
        mat->dense_fm_matrix->values[mat->fm_matrix_columns * mat->accumulation_matrix_rows + mat->rows_less_constraint_rows * DIMENSION + mat->accumulation_row_shift + k] = pressure_constraint_rhs_vector[(int)((frame_sample + k) / mat->dynamic_state_samples_per_frame)];
    	frame_sample++;
    }
}
...
void accumulate_constraint_into_dense_target_vector(MATRIX_DATA* mat, int frame_index, double constraint_element)
{
    mat->dense_fm_rhs_vector[mat->rows_less_constraint_rows * DIMENSION + frame_index] += constraint_element;
}

void accumulate_constraint_into_accumulation_target_vector(MATRIX_DATA* mat, int frame_index, double constraint_element)
{
	mat->dense_fm_matrix->add_scalar(DIMENSION * mat->rows_less_constraint_rows + frame_index, mat->accumulation_matrix_rows, constraint_element); 
}
...
int get_n_nonzero_matrix_elements(MATRIX_DATA* const mat)
{
    ...
    for (int k = 0; k < mat->rows_less_constraint_rows; k++) {
    ...
    if (mat->virial_constraint_rows > 0) {
        ...
        for (int k = 0; k < mat->virial_constraint_rows; k++) {
            for (int l = 0; l < mat->fm_matrix_columns; l++) {
                ...
                csr_fm_matrix.row_sizes[mat->rows_less_constraint_rows * DIMENSION + k + 1] = row_counter + 1; // re-apply one-base to rows
            }
        }
    }
}
...
void solve_sparse_fm_normal_equations(MATRIX_DATA* const mat)
{
   ...
   if (mat->output_residual == 1) {
    ...
    if (mat->bayesian_flag == 1 || mat->bayesian_flag == 2) {
	    ...
	    double n_cg_sites = (double)( mat->rows_less_constraint_rows/ mat->frames_per_traj_block / mat->size_per_vector);
...
void calculate_new_ibi_parameters(MATRIX_DATA* const mat_cg, MATRIX_DATA* const mat_ref)
{
  write_reference_matrix(mat_ref->dense_fm_normal_matrix);
  double beta = 1.0 / (mat_cg->temperature * mat_cg->boltzmann);
  update_these_ibi_parameters(beta, mat_cg->iteration_step_size, mat_ref->dense_fm_normal_matrix, mat_cg->dense_fm_normal_matrix, mat_ref->previous_rem_solution, mat_cg->previous_rem_solution, mat_cg->fm_solution);
}
...
void calculate_new_ibi_parameters_and_bootstrap(MATRIX_DATA* const mat_cg, MATRIX_DATA* const mat_ref)
{
  double beta = 1.0 / (mat_cg->temperature * mat_cg->boltzmann);
  double chi = mat_cg->iteration_step_size;
  std::vector<double> back_previous_solution(mat_ref->fm_matrix_columns);
  write_reference_matrix(mat_ref->dense_fm_normal_matrix);
  // copy previous_rem_solution to bootstrapping copies
  for (unsigned l = 0; l < mat_ref->previous_rem_solution.size(); l++) {
    back_previous_solution[l] = mat_ref->previous_rem_solution[l];
  }
  
  // update for master
  update_these_ibi_parameters(beta, chi, mat_ref->dense_fm_normal_matrix, mat_cg->dense_fm_normal_matrix, mat_ref->previous_rem_solution, mat_cg->previous_rem_solution, mat_cg->fm_solution);

  // update for bootstrap copies
  for (int k = 0; k < mat_cg->bootstrapping_num_estimates; k++) {
  	// Restore original previous_rem_solution
  	for (unsigned l = 0; l < mat_ref->previous_rem_solution.size(); l++) {
    	mat_ref->previous_rem_solution[l] = back_previous_solution[l];
  	}
    // Note: this assumes that the previous_rem_solution vectors are not needed later (since the get rewritten each iteration)    
    update_these_ibi_parameters(beta, chi, mat_ref->dense_fm_normal_matrix, mat_cg->bootstrapping_dense_fm_normal_matrices[k], mat_ref->previous_rem_solution, mat_cg->previous_rem_solution, mat_cg->bootstrap_solutions[k]);
  }
}
...
{ 
  ...
  for (int i = 0; i < icomp->ispec->n_defined; i++) {
	...
	if( icomp->ispec->class_type == kPairNonbonded ) {
	  ...
	  read_other_distribution(icomp, name, mat, i, counter,1.0);
	}
  }  
  if (icomp->ispec->class_type == kPairNonbonded) {
    delete [] sitecounter;
